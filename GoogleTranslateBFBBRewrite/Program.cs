...
namespace GoogleTranslateBFBBRewrite
{
    public class Config
    {
        ...
        {
            return HashCode.Combine(ExtractedGameFilesPath, HeavyModManagerFolderPath, TemporaryPath, TranslationIterations, MaxRetries);
        }
    }
    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                config = new Config { ExtractedGameFilesPath = "", HeavyModManagerFolderPath = "", TemporaryPath = "temp", TranslationIterations = 0, MaxRetries = 5 };
            ...
            foreach (string hipFile in hipFiles)
            {
                ...
                {
                    Console.WriteLine($"Skipping {fileNameNoExt} because it is in the exceptions list...");
                    continue;
                }
            ...
            Console.WriteLine($"BFBB Google Translator script completed in: {elapsedSeconds:F2} seconds");
            Console.WriteLine("The mod has been generated inside the HMM mods folder.");
            Console.WriteLine("It is ready to play! Enjoy! :)");
        ...
        public static string BulkTranslate(string str, int amount, Config config)
        {
            ...
            for (int i = 0; i < amount; i++)
            {
                ...
                for (int attempt = 1; attempt <= config.MaxRetries; attempt++)
                {
                    ...
                    {
                        Console.WriteLine($"Translation attempt {attempt} from {prevLang} to {nextLang} failed! Ex: {e.ToString()}");
                    }
                }
